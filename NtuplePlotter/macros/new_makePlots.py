...
def saveAccTemplates(inputDir, outFileName):
	...
	AccTemplates['TTGamma_presel'] = distribution('TTGamma_presel', [
		(inputDir+'hist_1phoNoMET_top_TTGamma.root', 1.0),
		], varList, 97)
	AccTemplates['TTJets1l'] = distribution('TTJets1l_presel', [
		(inputDir+'hist_1phoNoMET_top_TTJets1l.root', 1.0),
		], varList ,11)
	AccTemplates['TTJets2l'] = distribution('TTJets2l_presel', [
		(inputDir+'hist_1phoNoMET_top_TTJets2l.root', 1.0),
		], varList ,11)
	AccTemplates['TTJetsHad'] = distribution('TTJetsHad_presel', [
		(inputDir+'hist_1phoNoMET_top_TTJetsHad.root', 1.0),
		], varList ,11)
...
def saveBarrelFitTemplates(inputDir, inputData,  outFileName):
	...
	MCTempl_rs_b = loadMCTemplates(varList, inputDir, 'hist_1pho_barrel_top_', '_signal', 1001)
	MCTempl_fe_b = loadMCTemplates(varList, inputDir, 'hist_1pho_barrel_top_', '_electron', 3005)
	MCTempl_fjrb_b = loadMCTemplates(varList, inputDir, 'hist_1pho_barrel_top_', '_fake', 3005)
...
def makeAllPlots(varList, inputDir, dataDir, outDirName):
	...
	DataTempl = loadDataTemplate(varList, dataDir, 'hist_1phoNoMET_top_') #NoMET
	MCTemplDict = loadMCTemplates(varList, inputDir, 'hist_1phoNoMET_top_','',1001) #NoMET
	MCTempl = []
	MCTempl.append(MCTemplDict['WHIZARD'])
	MCTempl.append(MCTemplDict['TTJets'])
	MCTempl.append(MCTemplDict['Vgamma'])
	...
	MCTempl_rs_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_barrel_top_', '_signal', 1001)
	MCTempl_fe_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_barrel_top_', '_electron', 3005)
	MCTempl_fjrb_b = loadMCTemplates(shortVarList, inputDir, 'hist_1pho_barrel_top_', '_fake', 3005)
...
savePreselTemplates(InputHist, DataHist, 'templates_presel.root')
